...
func TestPing2Nodes(t *testing.T) {
	...
	res0, err := n1.PorcelainAPI.NetworkPing(ctx, n2.Network3140.PeerHost.ID())
	...
	res1, err := n1.PorcelainAPI.NetworkPing(ctx, n2.Network3140.PeerHost.ID())
	assert.NoError(t, err)
	assert.NoError(t, (<-res1).Error)

	res2, err := n2.PorcelainAPI.NetworkPing(ctx, n1.Network3140.PeerHost.ID())
	assert.NoError(t, err)
