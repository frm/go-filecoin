...
func TestPing2Nodes(t *testing.T) {
	...
	res0, err := n1.Refactor3140.PorcelainAPI.NetworkPing(ctx, n2.PeerHost.ID())
	...
	res1, err := n1.Refactor3140.PorcelainAPI.NetworkPing(ctx, n2.PeerHost.ID())
	assert.NoError(t, err)
	assert.NoError(t, (<-res1).Error)

	res2, err := n2.Refactor3140.PorcelainAPI.NetworkPing(ctx, n1.PeerHost.ID())
	assert.NoError(t, err)
