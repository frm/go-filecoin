...
func (node *Node) AddNewBlock(ctx context.Context, b *types.Block) (err error) {
	...
	if err := node.Chain3140.Syncer.HandleNewTipSet(ctx, types.NewChainInfo(node.Host().ID(), types.NewTipSetKey(blkCid), uint64(b.Height)), trusted); err != nil {
		return err
	}

	return node.PorcelainAPI.PubSubPublish(net.BlockTopic(node.Network3140.NetworkName), b.ToNode().RawData())
}
...
func (node *Node) processBlock(ctx context.Context, pubSubMsg pubsub.Message) (err error) {
	...
	// to address in #2674
	trusted := true
	err = node.Chain3140.Syncer.HandleNewTipSet(ctx, types.NewChainInfo(from, types.NewTipSetKey(blk.Cid()), uint64(blk.Height)), trusted)
	if err != nil {
