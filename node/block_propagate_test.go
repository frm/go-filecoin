...
func requireMineOnce(ctx context.Context, t *testing.T, minerNode *Node) *types.Block {
	head := minerNode.Chain3140.ChainReader.GetHead()
	headTipSet, err := minerNode.Chain3140.ChainReader.GetTipSet(head)
...
func TestBlockPropsManyNodes(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		for j := 1; j < numNodes; j++ {
			otherHead := nodes[j].Chain3140.ChainReader.GetHead()
...
func TestChainSync(t *testing.T) {
	...
	equal := false
	for i := 0; i < 30; i++ {
		otherHead := nodes[1].Chain3140.ChainReader.GetHead()
		assert.NotNil(t, otherHead)
