...
func TestConnectsToBootstrapNodes(t *testing.T) {
	...
	t.Run("connects to bootstrap nodes", func(t *testing.T) {
		...
		nd, err := node.New(ctx, opts...)
		require.NoError(t, err)
		nd.Network3140.Bootstrapper.MinPeerThreshold = 2
		nd.Network3140.Bootstrapper.Period = 10 * time.Millisecond
		assert.NoError(t, nd.Start(ctx))
		defer nd.Stop(ctx)
...
func TestNodeInit(t *testing.T) {
	...
	assert.NotEqual(t, 0, nd.Chain3140.ChainReader.GetHead().Len())
	nd.Stop(ctx)
}
